...
const LS_CENTRAL_CACHE="dt1_central_database_cache_v320";
...
async function loadCentralDatabase(force=false){
  ...
  try{
    status.textContent=force?"Rechargement de la base centrale...":"Chargement de la base centrale...";
    const res=await fetch(`${CENTRAL_DB_URL}?v=${APP_VERSION}`,{cache:"no-store"});
    ...
    // Safari limite fortement localStorage. On conserve seulement une copie légère,
    // sans les photos intégrées, afin qu'un dépassement de quota ne fasse jamais
    // échouer le chargement de la vraie base.
    try{
      const lightItems=centralItems.map(x=>({...x,photo:""}));
      localStorage.setItem(LS_CENTRAL_CACHE,JSON.stringify({items:lightItems,updatedAt:new Date().toISOString()}));
    }catch(cacheError){console.warn("Cache local léger non enregistré",cacheError);}
    return;
  }catch(error){
    console.warn("Chargement de database.json impossible",error);
    ...
    if(cached){
      try{
        ...
        return;
      }catch(cacheError){console.warn("Cache local invalide",cacheError);}
    }
...
function updatePreview(){const r=selectedItem(), box=document.getElementById("recipePreview"); if(!r){box.classList.add("hidden");return} document.getElementById("previewImg").src=photoOrPlaceholder(r); document.getElementById("previewName").textContent=r.name; document.getElementById("previewCarbs").textContent=`${String(r.carbs).replace(".",",")} g de glucides nets / 100 g`; document.getElementById("previewCategory").textContent=r.category; document.getElementById("previewSource").textContent=r.source==="central"?"Base centrale":"Familial"; const pp=Number(r.carbsPerPortion)||(Number(r.portionCount)>0&&Number(r.totalCarbs)>=0?Number(r.totalCarbs)/Number(r.portionCount):0), ppBox=document.getElementById("previewPortionStat"); if(ppBox){ppBox.classList.toggle("hidden",!(pp>0)); document.getElementById("previewCarbsPerPortion").textContent=`${pp.toFixed(1).replace(".",",")} g`} const fav=document.getElementById("favoriteToggle"); fav.textContent=isFavorite(r.id)?"★ Favori":"☆ Ajouter aux favoris"; fav.classList.toggle("active",isFavorite(r.id)); box.classList.remove("hidden")}
...
function renderRecipeBuilder(){const list=document.getElementById("ingredientList"); if(recipeIngredients.length===0)list.innerHTML=`<div class="notice mini">Aucun ingrédient ajouté.</div>`; else list.innerHTML=recipeIngredients.map((ing,idx)=>`<div class="ingredient-row"><div><strong>${ing.name}</strong><small>${ing.qty} ${unitLabel(ing.unit)} ≈ ${Math.round(ing.grams)} g · ${Number(ing.carbs).toFixed(1).replace(".",",")} g glucides</small></div><button data-remove-ing="${idx}">×</button></div>`).join(""); const total=recipeIngredients.reduce((s,i)=>s+Number(i.carbs||0),0), autoWeight=recipeIngredients.reduce((s,i)=>s+Number(i.grams||0),0), weightEl=document.getElementById("finalRecipeWeight"); if(weightEl&&!finalWeightManualOverride){weightEl.value=autoWeight>0?Math.round(autoWeight*10)/10:""} const fw=parseFloat(weightEl?.value)||0, p=fw>0?total/fw*100:0, portions=parseFloat(document.getElementById("recipePortionCount")?.value)||0, perPortion=portions>0?total/portions:0; document.getElementById("recipeTotalCarbs").textContent=`${total.toFixed(1).replace(".",",")} g`; document.getElementById("recipeCarbsPer100").textContent=`${p.toFixed(1).replace(".",",")} g`; const portionStat=document.getElementById("recipePerPortionStat"); if(portionStat){portionStat.classList.toggle("hidden",portions<=0); document.getElementById("recipeCarbsPerPortion").textContent=`${perPortion.toFixed(1).replace(".",",")} g`}}
function clearRecipeBuilder(){recipeIngredients=[];currentRecipeIngredient=null;createdRecipePhotoData="";editingRecipeId="";finalWeightManualOverride=false;["newRecipeName","finalRecipeWeight","recipePortionCount","ingredientSearch","ingredientQty"].forEach(id=>{const e=document.getElementById(id); if(e)e.value=""}); document.getElementById("selectedIngredientBox").classList.add("hidden"); document.getElementById("createdRecipePhoto").value=""; document.getElementById("createdRecipePhotoPreview").classList.add("hidden"); document.getElementById("saveCreatedRecipeBtn").textContent="Enregistrer dans le registre"; renderRecipeBuilder()}
function saveCreatedRecipe(){const name=document.getElementById("newRecipeName").value.trim(), fw=parseFloat(document.getElementById("finalRecipeWeight").value)||0, portions=parseFloat(document.getElementById("recipePortionCount").value)||0, total=recipeIngredients.reduce((s,i)=>s+Number(i.carbs||0),0); if(!name){alert("Entre le nom de la recette.");return} if(!recipeIngredients.length){alert("Ajoute au moins un ingrédient.");return} if(fw<=0){alert("Entre le poids final de la recette complète en grammes.");return} if(portions<0){alert("Le nombre de portions doit être supérieur à zéro.");return} const per100=total/fw*100, perPortion=portions>0?total/portions:null, recipeData={name,carbs:Math.round(per100*10)/10,category:"Recette",source:"local",ingredients:recipeIngredients,totalCarbs:Math.round(total*10)/10,finalWeight:fw,portionCount:portions>0?portions:null,carbsPerPortion:perPortion!==null?Math.round(perPortion*10)/10:null}; if(editingRecipeId){const ex=localItems.find(x=>x.id===editingRecipeId); if(ex){Object.assign(ex,{...recipeData,photo:createdRecipePhotoData||ex.photo||""}); saveLocalItems(); const id=ex.id; clearRecipeBuilder(); renderRecipes(); selectItem(id); setTab("calc"); alert("Recette modifiée."); return}} const id=`local_recipe_${Date.now()}`; localItems.push({id,...recipeData,photo:createdRecipePhotoData||""}); saveLocalItems(); clearRecipeBuilder(); renderRecipes(); selectItem(id); setTab("calc"); alert("Recette enregistrée dans le registre.")}
...
const LIVIA_APP_VERSION = "3.2.0";
const LIVIA_VERSION_DATE = "2026-07-12";
